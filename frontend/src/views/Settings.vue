...
import { discoverModels, type ModelDiscoveryResult } from "../api/config";
import { useConfigStore } from "../stores/config";
import { mergeLlmForm, omitPreservedSecret, type LlmProvider, type LlmApiFormat } from "../composables/llmForm";
...
const form = reactive<LlmFormModel>({
  llm_provider: "ollama", llm_api_format: "ollama", llm_model: "qwen2.5:7b", llm_base_url: "http://ollama:11434", llm_api_key: "",
...
async function discover() {
  if (form.llm_provider !== "ollama" && !form.llm_api_key.trim()) return;
  if (!form.llm_base_url.trim()) return;
  discovery.loading = true;
  try {
    const result: ModelDiscoveryResult = await discoverModels(form, "config");
...
function applyConfig(config: Record<string, { value: string | number | boolean; is_set: boolean }>) {
  for (const key of ["llm_provider", "llm_api_format", "llm_model", "llm_base_url", "llm_temperature", "llm_max_tokens", "llm_timeout", "llm_max_retries"] as const) {
    const entry = config[key];
    if (entry) (form[key] as string | number) = entry.value as never;
  }
  migrationRequired.value = Boolean(config.llm_migration_required?.value);
...
<template>
  <div v-loading="loading" class="settings-view">
    ...
    <el-form label-position="top" class="settings-form">
      <section class="settings-section"><div class="section-heading"><div><span class="section-index">01</span><h3>AI 引擎</h3></div><button class="ghost-action" type="button" :disabled="testing" @click="testConnection"><span class="pulse-icon"></span>{{ testing ? "测试中" : "测试连接" }}</button></div><el-alert v-if="migrationRequired" type="warning" title="旧版 LLM 配置需要重新选择服务商" :closable="false" /><LlmConfigFields ref="llmFields" :model-value="form" :models="discovery.models" :model-loading="discovery.loading" :manual-model-input="discovery.manual" :model-error="discovery.error" @update:model-value="updateLlmForm" @discover="discover" /></section>
